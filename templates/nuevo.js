...
shooterImage.src = "nave.png"; // Cambia con la ruta de la imagen del tirador
...
class Duck {
    constructor() {
        this.width = 100;  // Ancho fijo del pato
        this.height = 100; // Alto fijo del pato
        ...
        this.image.src = "duck2.png"; // Cambia con la ruta de tu imagen de pato
        this.duckBullets = []; // Array para almacenar las balas de los patos
         // Disparar una bala cada 2 segundos
         setInterval(() => {
            this.shootBullet();
        }, 2000);
    }
    ...
    shootBullet() {
        this.duckBullets.push(new DuckBullet(this.x + this.width / 2 - 2, this.y + this.height));
    }
    ...
    draw() {
        ctx.drawImage(this.image, this.x, this.y, this.width, this.height);
          // Dibujar las balas disparadas por el pato
...
// Definición de la bala disparada por los patos
class DuckBullet {
    constructor(x, y) {
        this.x = x;
        this.y = y;
    ...
    move() {
        this.y += 13; // Dispara hacia abajo
    }

    draw() {
        ctx.fillStyle = "yellow"; // Puedes cambiar el color
        ctx.fillRect(this.x, this.y, this.width, this.height);
    }
}
...
function detectCollisions() {
    for (let b of bullets) {
        for (let d of ducks) {
            // Colisiones entre las balas del jugador y los patos
            if (b.x < d.x + d.width && b.x + b.width > d.x && b.y < d.y + d.height && b.y + b.height > d.y) {
                score += 10;
                document.getElementById('score').innerText = score;
                localStorage.setItem("gameScore", score);
                sonidoImpacto.currentTime = 0;
                sonidoImpacto.play();
                d.reset();
    ...
    // Colisiones entre las balas de los patos y el tirador
    for (let b of ducks) {
